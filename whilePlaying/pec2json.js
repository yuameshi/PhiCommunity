export function pec2json(pec, filename) {
	const tween = [
		null,
		null,
		...
		(pos) =>
			((pos *= 2) < 1
				? 1 - Math.sqrt(1 - pos ** 2)
				: Math.sqrt(1 - (pos - 2) ** 2) + 1) / 2, //22
		(pos) =>
			pos < 0.5
				? (14.379638 * pos - 5.189819) * pos ** 2
				: (14.379638 * pos - 9.189819) * (pos - 1) ** 2 + 1, //23
		...
		(pos) =>
			((pos *= 11) < 4
				...
				: pos < 10
				? (pos - 9) ** 2 + 15
				: (pos - 10.5) ** 2 + 15.75) / 16, //26
		(pos) => 1 - tween[26](1 - pos), //27
		(pos) =>
			(pos *= 2) < 1 ? tween[26](pos) / 2 : tween[27](pos - 1) / 2 + 0.5, //28
		(pos) =>
			pos < 0.5
				? 2 ** (20 * pos - 11) *
				  Math.sin(((160 * pos + 1) * Math.PI) / 18)
				: 1 -
				  2 ** (9 - 20 * pos) *
						Math.sin(((160 * pos + 1) * Math.PI) / 18), //29
	];
	class JudgeLine {
		bpm = 120;
		numOfNotes = 0;
		...
		judgeLineDisappearEventsPec = [];
		judgeLineMoveEventsPec = [];
	...
	const rawChart = pec
		.match(/[^\n\r ]+/g)
		.map((i) => (isNaN(i) ? String(i) : Number(i)));
	...
	function calcTime(timePec) {
		...
		for (const i of bpmEvents) {
			...
			timePhi = Math.round(
				((timePec - i.startTime) / i.bpm + i.value) * baseBpm * 32
			);
		}
		return timePhi;
	}
	...
	for (const i of raw.n1) {
		...
		linesPec[i[0]].pushNote(
			new Note(
				1,
				calcTime(i[1]) + (i[4] ? 1e9 : 0),
				(i[2] * 9) / 1024,
				0,
				i[5]
			),
	...
	for (const i of raw.n2) {
		...
		linesPec[i[0]].pushNote(
			new Note(
				3,
				calcTime(i[1]) + (i[5] ? 1e9 : 0),
				(i[3] * 9) / 1024,
				calcTime(i[2]) - calcTime(i[1]),
				i[6]
			),
			i[4],
			i[5]
	...
	for (const i of linesPec) {
		if (i) {
			...
			const srt = (a, b) =>
				a.startTime - b.startTime + (a.endTime - b.endTime); //不单独判断以避免误差
			...
			for (let j = 0; j < s.length; j++) {
				...
				s[j].floorPosition = y;
				y +=
					(((s[j].endTime - s[j].startTime) * s[j].value) / i.bpm) *
					1.875;
			}
			...
			for (const j of ldp) {
				...
				if (tween[j.motionType]) {
					for (
						let k = parseInt(j.startTime);
						k < parseInt(j.endTime);
						k++
					) {
						let ptt1 =
							(k - j.startTime) / (j.endTime - j.startTime);
						let ptt2 =
							(k + 1 - j.startTime) / (j.endTime - j.startTime);
						let pt1 = j.value - ldpValue;
						i.pushEvent(
							1,
							k,
							k + 1,
							ldpValue + tween[j.motionType](ptt1) * pt1,
							ldpValue + tween[j.motionType](ptt2) * pt1
						);
					}
				...
				if (tween[j.motionType]) {
					for (
						...
						i.pushEvent(
							...
							k,
							k + 1,
							lmpValue + tween[j.motionType](ptt1) * pt1,
							lmpValue + tween[j.motionType](ptt2) * pt1,
							lmpValue2 + tween[j.motionType](ptt1) * pt2,
							lmpValue2 + tween[j.motionType](ptt2) * pt2
						);
					}
				} else if (j.motionType)
					i.pushEvent(
				...
				if (tween[j.motionType]) {
					for (
						...
						i.pushEvent(
							...
							k,
							k + 1,
							lrpValue + tween[j.motionType](ptt1) * pt1,
							lrpValue + tween[j.motionType](ptt2) * pt1
						);
					}
				} else if (j.motionType)
